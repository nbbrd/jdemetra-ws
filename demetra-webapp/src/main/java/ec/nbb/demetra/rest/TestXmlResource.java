...
public class TestXmlResource {
    ...
    @ApiOperation(value = "[TEST] Returns the given JsonTsCollection", notes = "Returns the given XmlTsCollection object for serialization testing purpose", response = JsonTsCollection.class)
    @ApiResponses(
            value = {
                @ApiResponse(code = 200, message = "Object succesfully returned", response = JsonTsCollection.class),
                @ApiResponse(code = 400, message = "Bad request", response = String.class),
                @ApiResponse(code = 500, message = "Invalid request", response = String.class)
            }
    )
    public Response tsCollection(@ApiParam(value = "tsCollection", required = true) JsonTsCollection tsCollection) {
